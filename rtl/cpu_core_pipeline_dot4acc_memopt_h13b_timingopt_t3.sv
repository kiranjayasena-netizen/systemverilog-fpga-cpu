...
module cpu_core_pipeline_dot4acc_memopt_h13b_timingopt_t3 #(
    ...
    // T3: an EX redirect cancels the ID/EX transaction through its valid bit.
    // Global reset and ordinary hazard/hold bubbles continue to use the full
    // clear_id_ex task above.
    task automatic invalidate_id_ex();
        begin
            id_ex_reg.valid <= 1'b0;
        end
    endtask
...
`ifndef SYNTHESIS
    ...
    always @(posedge clk) begin
        ...
        if (!rst) begin
            ...
            // T3 valid-bit contract: an invalid ID/EX payload cannot create
            // an architectural effect or a new control decision.
            assert (!(!id_ex_reg.valid && dot_issue_accept))
                else $fatal(1, "invalid ID/EX issued DOT");
            assert (!(!id_ex_reg.valid && ex_redirect_taken))
                else $fatal(1, "invalid ID/EX redirected");
