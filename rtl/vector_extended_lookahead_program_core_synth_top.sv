module vector_extended_lookahead_program_core_synth_top #(
 ...
 vector_extended_lookahead_program_core #(.LOOKAHEAD_DEPTH(LOOKAHEAD_DEPTH)) core(
 ...
 assign status_parity=^current_instruction ^ current_instruction_valid ^ ^data_debug_read_data ^ ^debug_read_data ^ ^c0 ^ ^c1 ^ ^c11 ^ ^c12 ^ ^debug_entry0_data ^ ^debug_entry1_data;
endmodule
