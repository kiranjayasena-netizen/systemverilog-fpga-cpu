...
module cpu_core_pipeline_loadtiming #(
    ...
    // Phase 13D removes the decode-time WB-to-ID data bypass from the
    // data-memory result path. If decode needs the register currently being
    // written back, hold IF/ID for one cycle so the register file update is
    // visible before the instruction enters ID/EX.
    assign wb_decode_stall =
        if_id_reg.valid &&
        if_id_opcode_valid &&
        wb_writes_rd &&
        ((if_id_uses_rs1 && (if_id_rs1 == mem_wb_reg.rd)) ||
         (if_id_uses_rs2 && (if_id_rs2 == mem_wb_reg.rd)));
    ...
    assign decode_stall = load_use_stall || wb_decode_stall;
