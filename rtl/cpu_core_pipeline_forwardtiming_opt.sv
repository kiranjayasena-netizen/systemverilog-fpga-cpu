...
module cpu_core_pipeline_forwardtiming_opt #(
    ...
    // Phase 17C optimisation: an older EX-stage redirect has first priority
    // on the instruction-BRAM address path. This directly launches the taken
    // BEQ target request instead of waiting for the registered redirect launch
    // cycle used by the Phase 13E baseline. The existing ID-stage fast JUMP
    // path remains unchanged and is still suppressed by older EX redirects.
    ...
    always_comb begin
        if (ex_redirect_taken) begin
            next_instruction_addr = ex_redirect_target;
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            if (ex_redirect_taken) begin
                ...
                fetch_buffer_valid       <= 1'b0;
                fetch_pending_pc         <= ex_redirect_target;
                fetch_pending_valid      <= 1'b1;
                fetch_pc                 <= ex_redirect_target + 32'd4;
                redirect_pending_valid   <= 1'b0;
                redirect_pending_target  <= ex_redirect_target;
                jump_response_pending    <= 1'b0;
