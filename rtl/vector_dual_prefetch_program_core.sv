// Stage 10A genuine two-entry prefetch controller. Prefetching is limited to
// proven VSTORE/read overlap; ALU/read overlap is intentionally deferred.
module vector_dual_prefetch_program_core #(
 ...
 output logic [31:0] prefetch_request_count,prefetch_hit_count,prefetch_miss_count,entry0_hit_count,entry1_hit_count,overlap_alu_count,overlap_store_count,dependency_blocked_count,entry0_fill_count,entry1_fill_count,duplicate_suppression_count
 ,output logic debug_entry0_valid,debug_entry1_valid,output logic [4:0] debug_entry0_addr,debug_entry1_addr,output logic [127:0] debug_entry0_data,debug_entry1_data,output logic debug_pending_read_valid
);
 localparam int PCW=(PROGRAM_DEPTH<=1)?1:$clog2(PROGRAM_DEPTH);
 typedef enum logic [1:0] {S_EXEC,S_LOAD_WB} state_t; state_t state;
 logic[15:0] instruction; logic[3:0] alu_op; logic[2:0] src_a,src_b,dst; logic[4:0] mem_addr;
 logic valid,is_alu,is_vload,is_vstore; logic[4:0] active_length; logic[2:0] load_dst;
 logic[127:0] mem_read_data,alu_result; logic mem_read_enable,mem_write_enable,core_reg_write,core_load_write; logic[4:0] mem_read_addr,mem_write_addr; logic[127:0] mem_write_data;
 logic entry0_valid,entry1_valid; logic[4:0] entry0_addr,entry1_addr; logic[127:0] entry0_data,entry1_data;
 logic pending_read_valid,pending_read_slot; logic[4:0] pending_read_addr; logic pending_discard;
 logic prefetch_issue,prefetch_hit0,prefetch_hit1,prefetch_hit; logic[4:0] candidate_addr; logic candidate_valid; logic candidate_slot;
 logic c1,c2,c3,c4;
 logic[15:0] next_instruction,next2_instruction,next3_instruction,next4_instruction; logic[3:0] next_pc; logic next_valid,next_is_vload,next_is_vstore,next2_valid,next2_is_vload,next2_is_vstore,next3_valid,next3_is_vload,next3_is_vstore,next4_valid,next4_is_vload,next4_is_vstore; logic[4:0] next_mem_addr,next2_mem_addr,next3_mem_addr,next4_mem_addr;
 ...
 // A buffered address is a hit only for the architectural VLOAD currently
 // executing.  ALU/invalid instructions decode mem_addr as zero; allowing
 // those slots to assert a hit would incorrectly give load-write priority
 // over an ALU result when a prefetched address 0 is still buffered.
 assign prefetch_hit0=is_vload&&entry0_valid&&entry0_addr==mem_addr;
 assign prefetch_hit1=is_vload&&entry1_valid&&entry1_addr==mem_addr;
 assign prefetch_hit=prefetch_hit0||prefetch_hit1;
 // Conservative candidate: the immediate next slot only. This is sufficient
 // to demonstrate independent entries while preserving store ordering.
 assign c1=(current_pc+1'b1<active_length)&&next_valid&&next_is_vload&&(next_mem_addr!=mem_addr)&&!(entry0_valid&&entry0_addr==next_mem_addr)&&!(entry1_valid&&entry1_addr==next_mem_addr)&&!(pending_read_valid&&pending_read_addr==next_mem_addr);
 assign c2=(current_pc+2'd2<active_length)&&next2_valid&&next2_is_vload&&(next2_mem_addr!=mem_addr)&&!(next_valid&&next_is_vstore&&next_mem_addr==next2_mem_addr)&&!(entry0_valid&&entry0_addr==next2_mem_addr)&&!(entry1_valid&&entry1_addr==next2_mem_addr)&&!(pending_read_valid&&pending_read_addr==next2_mem_addr);
 assign c3=(current_pc+2'd3<active_length)&&next3_valid&&next3_is_vload&&(next3_mem_addr!=mem_addr)&&!(next_valid&&next_is_vstore&&next_mem_addr==next3_mem_addr)&&!(next2_valid&&next2_is_vstore&&next2_mem_addr==next3_mem_addr)&&!(entry0_valid&&entry0_addr==next3_mem_addr)&&!(entry1_valid&&entry1_addr==next3_mem_addr)&&!(pending_read_valid&&pending_read_addr==next3_mem_addr);
 assign c4=(current_pc+3'd4<active_length)&&next4_valid&&next4_is_vload&&(next4_mem_addr!=mem_addr)&&!(next_valid&&next_is_vstore&&next_mem_addr==next4_mem_addr)&&!(next2_valid&&next2_is_vstore&&next2_mem_addr==next4_mem_addr)&&!(next3_valid&&next3_is_vstore&&next3_mem_addr==next4_mem_addr)&&!(entry0_valid&&entry0_addr==next4_mem_addr)&&!(entry1_valid&&entry1_addr==next4_mem_addr)&&!(pending_read_valid&&pending_read_addr==next4_mem_addr);
 assign candidate_valid=running&&state==S_EXEC&&is_vstore&&!pending_read_valid&&(c1||c2||c3||c4);
 assign candidate_addr=c1?next_mem_addr:c2?next2_mem_addr:c3?next3_mem_addr:next4_mem_addr; assign candidate_slot=!entry0_valid?1'b0:1'b1;
 ...
 assign prefetch_issue=candidate_valid&&(!entry0_valid||!entry1_valid)&&!(entry0_valid&&entry0_addr==candidate_addr)&&!(entry1_valid&&entry1_addr==candidate_addr);
 ...
 always_ff @(posedge clk) begin
  if(rst) begin current_pc<='0;running<=0;done<=0;active_length<='0;load_dst<='0;state<=S_EXEC;entry0_valid<=0;entry1_valid<=0;pending_read_valid<=0;pending_discard<=0;prefetch_request_count<=0;prefetch_hit_count<=0;prefetch_miss_count<=0;entry0_hit_count<=0;entry1_hit_count<=0;overlap_alu_count<=0;overlap_store_count<=0;dependency_blocked_count<=0;entry0_fill_count<=0;entry1_fill_count<=0;duplicate_suppression_count<=0; end
  else begin
   if(!running&&(data_load_enable||data_debug_read_enable)) begin entry0_valid<=0;entry1_valid<=0;pending_read_valid<=0; end
   ...
   if(prefetch_issue) begin pending_read_valid<=1;pending_read_addr<=candidate_addr;pending_read_slot<=candidate_slot;prefetch_request_count<=prefetch_request_count+1'b1;overlap_store_count<=overlap_store_count+1'b1;end
   ...
   if(!running) begin if(start) begin current_pc<='0;entry0_valid<=0;entry1_valid<=0;pending_read_valid<=0;prefetch_request_count<=0;prefetch_hit_count<=0;prefetch_miss_count<=0;entry0_hit_count<=0;entry1_hit_count<=0;overlap_alu_count<=0;overlap_store_count<=0;dependency_blocked_count<=0;entry0_fill_count<=0;entry1_fill_count<=0;duplicate_suppression_count<=0;if(program_length==0||program_length>PROGRAM_DEPTH)begin running<=0;done<=1;end else begin active_length<=program_length;running<=1;done<=0;state<=S_EXEC;end end end
