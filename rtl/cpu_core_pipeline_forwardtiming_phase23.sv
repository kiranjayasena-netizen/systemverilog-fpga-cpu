...
module cpu_core_pipeline_forwardtiming_phase23 #(
    ...
    logic        jump_cache_valid;
    logic [31:0] jump_cache_pc;
    logic [31:0] jump_cache_target;
    logic        jump_lookup_hit_reg;
    logic [31:0] jump_lookup_target_reg;
    logic        jump_cache_response_pending;
    ...
    logic        id_jump_cached_response_ready;
    logic        jump_predict_request;
    ...
    // Phase 13E keeps the Phase 13C WB-to-ID bypass behaviour, but splits
    ...
    assign jump_predict_request = jump_lookup_hit_reg &&
                                  fetch_pending_valid &&
                                  !fetch_buffer_valid &&
                                  !jump_response_pending;
    assign id_jump_cached_response_ready = id_jump_redirect &&
                                           jump_cache_response_pending &&
                                           fetch_pending_valid &&
                                           (fetch_pending_pc == id_jump_target);
    ...
    always_comb begin
        if (id_jump_fast_request && !id_jump_cached_response_ready) begin
            next_instruction_addr = id_jump_target;
        end else if (id_jump_cached_response_ready && !enable) begin
            next_instruction_addr = id_jump_target;
        end else if (id_jump_cached_response_ready) begin
            next_instruction_addr = fetch_pc;
        end else if (jump_predict_request && enable) begin
            next_instruction_addr = jump_lookup_target_reg;
        end else if (jump_predict_request && !enable) begin
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            jump_cache_valid         <= 1'b0;
            jump_cache_pc            <= 32'h0000_0000;
            jump_cache_target        <= 32'h0000_0000;
            jump_lookup_hit_reg      <= 1'b0;
            jump_lookup_target_reg   <= 32'h0000_0000;
            jump_cache_response_pending <= 1'b0;
            ...
            if (ex_redirect_taken) begin
                ...
                jump_cache_valid         <= 1'b1;
                jump_cache_pc            <= if_id_reg.pc;
                jump_cache_target        <= id_jump_target;

                if (id_jump_cached_response_ready) begin
                    if_id_reg.valid       <= opcode_is_valid(bram_instruction[31:28]);
                    if_id_reg.pc          <= id_jump_target;
                    ...
                    jump_response_pending    <= 1'b0;
                    jump_lookup_hit_reg      <= jump_cache_valid && (fetch_pc == jump_cache_pc);
                    jump_lookup_target_reg   <= jump_cache_target;
                    jump_cache_response_pending <= 1'b0;
                end else begin
                    if_id_reg.valid       <= 1'b0;
                ...
                if (jump_predict_request) begin
                    fetch_pending_pc            <= jump_lookup_target_reg;
                    fetch_pending_valid         <= 1'b1;
                    fetch_pc                    <= jump_lookup_target_reg + 32'd4;
                    jump_lookup_hit_reg         <= 1'b0;
                    jump_cache_response_pending <= 1'b1;
                end else begin
                    fetch_pending_pc            <= fetch_pc;
                    fetch_pending_valid         <= 1'b1;
                    fetch_pc                    <= fetch_pc + 32'd4;
