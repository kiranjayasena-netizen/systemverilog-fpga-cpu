...
module cpu_core_pipeline_branchprefetch #(
    ...
    output logic [31:0] beq_target_prefetch_requests,
    output logic [31:0] beq_target_prefetch_hits,
    output logic [31:0] beq_target_prefetch_discards,
    output logic [31:0] beq_target_prefetch_invalidations
);
    ...
    logic [31:0] bram_branch_instruction;
    ...
    logic        beq_prefetch_pending;
    logic [31:0] beq_prefetch_branch_pc;
    logic [31:0] beq_prefetch_target_pc;
    ...
    logic        id_beq_prefetch_addr_request;
    logic        id_beq_prefetch_request;
    logic        beq_prefetch_hit;
    ...
    logic [31:0] branch_prefetch_addr;
    ...
    assign if_id_branch_target = if_id_reg.pc + (if_id_imm_ext << 2);
    ...
    assign ex_redirect_target = id_ex_reg.branch ?
                                id_ex_reg.branch_target :
                                (id_ex_reg.pc + (id_ex_reg.imm_ext << 2));
    ...
    // The speculative BEQ target address is driven from decode-only metadata.
    // The older EX-stage branch comparison is deliberately not part of this
    // address path; if an older redirect wins, the speculative Port B response
    // has no saved metadata and is ignored.
    assign id_beq_prefetch_addr_request = if_id_reg.valid &&
                                          if_id_opcode_valid &&
                                          (if_id_opcode == OP_BEQ) &&
                                          !decode_stall &&
                                          !jump_response_pending &&
                                          !redirect_pending_valid;
    assign id_beq_prefetch_request = id_beq_prefetch_addr_request && !ex_redirect_taken;
    assign beq_prefetch_hit = ex_branch_taken &&
                              beq_prefetch_pending &&
                              (beq_prefetch_branch_pc == id_ex_reg.pc);
    ...
    always_comb begin
        if (id_beq_prefetch_addr_request) begin
            branch_prefetch_addr = if_id_branch_target;
        end else if (beq_prefetch_pending) begin
            branch_prefetch_addr = beq_prefetch_target_pc;
        end else begin
            branch_prefetch_addr = 32'h0000_0000;
        end
    end
    ...
    bram_instr_mem_dualread #(
        ...
        .clk(clk),
        .addr_a(instruction_addr),
        .addr_b(branch_prefetch_addr),
        .instruction_a(bram_instruction),
        .instruction_b(bram_branch_instruction)
    );
    ...
    task automatic clear_id_ex();
        begin
            ...
            id_ex_reg.branch_target <= 32'h0000_0000;
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            beq_prefetch_pending     <= 1'b0;
            beq_prefetch_branch_pc   <= 32'h0000_0000;
            beq_prefetch_target_pc   <= 32'h0000_0000;
            ...
            beq_target_prefetch_requests    <= 32'd0;
            beq_target_prefetch_hits        <= 32'd0;
            beq_target_prefetch_discards    <= 32'd0;
            beq_target_prefetch_invalidations <= 32'd0;
            ...
            if (id_ex_reg.valid && id_ex_reg.branch) begin
                if (ex_branch_taken) begin
                    ...
                    if (beq_prefetch_hit) begin
                        beq_target_prefetch_hits <= beq_target_prefetch_hits + 32'd1;
                    end
                end else begin
                    not_taken_branches <= not_taken_branches + 32'd1;
                    if (beq_prefetch_pending &&
                        (beq_prefetch_branch_pc == id_ex_reg.pc)) begin
                        beq_target_prefetch_discards <= beq_target_prefetch_discards + 32'd1;
                        beq_prefetch_pending   <= 1'b0;
                        beq_prefetch_branch_pc <= 32'h0000_0000;
                        beq_prefetch_target_pc <= 32'h0000_0000;
                    end
            ...
            if (ex_redirect_taken || decode_stall) begin
                ...
                id_ex_reg.branch_target <= if_id_branch_target;
                ...
                if (if_id_opcode == OP_BEQ) begin
                    beq_prefetch_pending   <= 1'b1;
                    beq_prefetch_branch_pc <= if_id_reg.pc;
                    beq_prefetch_target_pc <= if_id_branch_target;
                    beq_target_prefetch_requests <= beq_target_prefetch_requests + 32'd1;
                end
            end else begin
                clear_id_ex();
            end
            ...
            if (ex_redirect_taken) begin
                ...
                if (ex_branch_taken && beq_prefetch_hit) begin
                    if_id_reg.valid       <= opcode_is_valid(bram_branch_instruction[31:28]);
                    if_id_reg.pc          <= beq_prefetch_target_pc;
                    if_id_reg.instruction <= bram_branch_instruction;
                    fetch_pc              <= beq_prefetch_target_pc + 32'd4;
                    redirect_pending_valid  <= 1'b0;
                    redirect_pending_target <= 32'h0000_0000;
                    beq_prefetch_pending    <= 1'b0;
                    beq_prefetch_branch_pc  <= 32'h0000_0000;
                    ...
                    if (beq_prefetch_pending) begin
                        beq_target_prefetch_invalidations <= beq_target_prefetch_invalidations + 32'd1;
                        beq_prefetch_pending   <= 1'b0;
                        beq_prefetch_branch_pc <= 32'h0000_0000;
                        beq_prefetch_target_pc <= 32'h0000_0000;
                    end
