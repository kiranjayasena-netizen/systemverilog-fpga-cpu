...
// Experimental Stage C core copied from cpu_core_pipeline_mac8_timingopt.
// It adds DOT4ACC decode, registered issue/dependency control and
// verification-only completion observation. DOT4ACC results never update
// architectural state or retirement in this core.
module cpu_core_pipeline_dot4acc_issue #(
    ...
    // Stage C verification-only DOT4ACC observation. None of these signals
    // feed architectural writeback, memory side effects, or retirement.
    ...
    function automatic logic stagec_opcode_writes_rd(input logic [3:0] opcode);
        // DOT4ACC deliberately stops before architectural writeback in Stage C.
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            if (redirect_taken) begin
                ...
                // A real non-DOT instruction ends contiguity. If a DOT is
                // stalled in ID/EX, defer the break until that DOT accepts so
                // its already-established chain classification is preserved.
            ...
            // DOT4ACC terminates at its private issue path. It can never enter
            // MEM/WB, write architectural state, or generate a retire event.
