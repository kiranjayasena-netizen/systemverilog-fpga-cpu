...
module vector_extended_lookahead_program_core #(
...
assign c5=(LOOKAHEAD_DEPTH>=5)&&(current_pc+3'd5<active_length)&&next5_valid&&next5_is_vload&&(!is_vstore||(next5_mem_addr!=mem_addr))&&!(next_valid&&next_is_vstore&&next_mem_addr==next5_mem_addr)&&!(next2_valid&&next2_is_vstore&&next2_mem_addr==next5_mem_addr)&&!(next3_valid&&next3_is_vstore&&next3_mem_addr==next5_mem_addr)&&!(next4_valid&&next4_is_vstore&&next4_mem_addr==next5_mem_addr)&&!(entry0_valid&&entry0_addr==next5_mem_addr)&&!(entry1_valid&&entry1_addr==next5_mem_addr)&&!(pending_read_valid&&pending_read_addr==next5_mem_addr);
