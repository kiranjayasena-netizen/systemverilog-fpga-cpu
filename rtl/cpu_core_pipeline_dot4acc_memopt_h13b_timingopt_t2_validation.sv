...
module cpu_core_pipeline_dot4acc_memopt_h13b_timingopt_t2_validation #(
    ...
    input  logic [7:0]  validation_prog_addr,
    input  logic [31:0] validation_prog_data,
    input  logic        validation_data_we,
    input  logic [7:0]  validation_data_addr,
    input  logic [31:0] validation_data_wdata,
    input  logic        validation_data_re,
    input  logic [7:0]  validation_data_raddr,
    output logic [31:0] validation_data_rdata,
    ...
    logic        load_overlap_allowed;
    logic        load_overlap_reserved;
    logic        dot_arch_pending;
    logic        dot_load_completion_deferred;
    logic        scalar_overlap_hold;
    logic        dot_deferred_rs1_match;
    logic        dot_deferred_rs2_match;
    logic        dot_deferred_acc_match;
    logic        dot_deferred_issue_exception;
    logic        second_load_reserved;
    logic        second_load_idex_owned;
    logic        second_load_admit_allowed;
    logic        second_load_holding;
    logic        first_load_overlap_context_active;
    logic        second_load_ifid_allow;
    logic        second_load_transfer_fire;
    logic        load_overlap_transfer_fire;
    logic        deferred_load_valid;
    logic [31:0] deferred_load_pc;
    logic [3:0]  deferred_load_opcode;
    logic [4:0]  deferred_load_rd;
    logic [31:0] deferred_load_data;
    logic [31:0] deferred_load_addr;
    logic        dot_idex_consumed;
    logic [31:0] dot_consumed_instruction;
    logic [31:0] dot_consumed_pc;
    ...
    assign rf_write_enable = enable &&
                             (dot_wb_valid ||
                              (!dot_load_completion_deferred &&
                               !deferred_load_valid && normal_wb_valid));
    ...
    assign dot_deferred_rs1_match = deferred_load_valid &&
                                    (deferred_load_rd != 5'd0) &&
                                    id_ex_is_dot &&
                                    (deferred_load_rd == id_ex_reg.rs1);
    assign dot_deferred_rs2_match = deferred_load_valid &&
                                    (deferred_load_rd != 5'd0) &&
                                    id_ex_is_dot &&
                                    (deferred_load_rd == id_ex_reg.rs2);
    assign dot_deferred_acc_match = deferred_load_valid &&
                                    (deferred_load_rd != 5'd0) &&
                                    id_ex_is_dot &&
                                    (deferred_load_rd == id_ex_reg.rd);
    // This is the only H1.2 relaxation of the H1.1 scalar hold.  It applies
    // to a ready DOT in ID/EX only; all younger non-DOT instructions and all
    // new LOADs remain held by the existing policy.
    assign dot_deferred_issue_exception = deferred_load_valid &&
                                          id_ex_is_dot &&
                                          dot_operands_ready &&
                                          !dot_deferred_acc_match;
    ...
    assign dot_issue_accept = enable && !rst && id_ex_is_dot &&
                              ...
                              !(dot_idex_consumed &&
                                (id_ex_reg.pc == dot_consumed_pc) &&
                                (id_ex_reg.instruction == dot_consumed_instruction));
    assign dot_issue_stall = id_ex_is_dot && !dot_operands_ready;

    assign dot_arch_pending = dot_idex_consumed || (|dot_inflight_valid) ||
                              dot_complete_valid;
    assign load_overlap_allowed = if_id_reg.valid && if_id_opcode_valid &&
                                  (if_id_opcode == OP_LOAD) &&
                                  dot_arch_pending &&
                                  !load_overlap_reserved &&
                                  !deferred_load_valid &&
                                  !(if_id_rd != 5'd0 &&
                                    ((dot_complete_valid && (if_id_rd == dot_complete_rd)) ||
                                     (dot_issue_reg.valid && (if_id_rd == dot_issue_reg.rd)) ||
                                     (dot_meta_stage1_reg.valid && (if_id_rd == dot_meta_stage1_reg.rd)) ||
                                     (dot_meta_stage2_reg.valid && (if_id_rd == dot_meta_stage2_reg.rd)))) &&
                                  !(if_id_rs1 != 5'd0 &&
                                    ((dot_complete_writable && (if_id_rs1 == dot_complete_rd)) ||
                                     (dot_issue_reg.valid && (if_id_rs1 == dot_issue_reg.rd)) ||
                                     (dot_meta_stage1_reg.valid && (if_id_rs1 == dot_meta_stage1_reg.rd)) ||
                                     (dot_meta_stage2_reg.valid && (if_id_rs1 == dot_meta_stage2_reg.rd))));
    // The first overlap transaction remains architecturally represented by
    // any of these states until its deferred/normal retirement has drained.
    assign first_load_overlap_context_active = load_overlap_reserved ||
                                               dot_load_completion_deferred ||
                                               deferred_load_valid;
    assign second_load_admit_allowed = if_id_reg.valid &&
                                       if_id_opcode_valid &&
                                       (if_id_opcode == OP_LOAD) &&
                                       first_load_overlap_context_active &&
                                       deferred_load_valid &&
                                       !second_load_reserved &&
                                       !redirect_pending_valid &&
                                       !ex_redirect_taken &&
                                       !(if_id_rd != 5'd0 &&
                                         ((dot_complete_valid && (if_id_rd == dot_complete_rd)) ||
                                       ...
                                       !(if_id_rs1 != 5'd0 &&
                                         ((dot_complete_writable && (if_id_rs1 == dot_complete_rd)) ||
                                          ...
                                          (dot_meta_stage1_reg.valid && (if_id_rs1 == dot_meta_stage1_reg.rd)) ||
                                          (dot_meta_stage2_reg.valid && (if_id_rs1 == dot_meta_stage2_reg.rd))));
    // Redirect and pending-redirect priority is enforced sequentially below;
    // keeping it out of this shared predicate removes the late branch-control
    // fanout from the frontend timing cone.
    assign load_overlap_transfer_fire = load_overlap_allowed &&
                                        !redirect_taken &&
                                        !redirect_pending_valid;
    assign second_load_ifid_allow = enable && !rst &&
                                    second_load_admit_allowed &&
                                    !ex_redirect_taken &&
                                    !dot_issue_stall;
    assign second_load_transfer_fire = second_load_ifid_allow;
    assign second_load_holding = second_load_idex_owned &&
                                 id_ex_reg.valid &&
                                 (id_ex_reg.opcode == OP_LOAD) &&
                                 deferred_load_valid;
    assign dot_active = id_ex_is_dot || dot_arch_pending;
    assign dot_younger_non_dot_hold = if_id_reg.valid &&
                                      if_id_opcode_valid &&
                                      !if_id_is_dot &&
                                      dot_active &&
                                      !(load_overlap_allowed ||
                                        second_load_admit_allowed);
    assign dot_frontend_hold = dot_issue_stall || dot_younger_non_dot_hold ||
                               (load_overlap_reserved && !deferred_load_valid);
    assign dot_load_completion_deferred = load_overlap_reserved &&
                                          mem_wb_reg.valid &&
                                          mem_wb_reg.mem_to_reg &&
                                          dot_arch_pending;
    assign scalar_overlap_hold = dot_load_completion_deferred ||
                                 (deferred_load_valid &&
                                  !dot_deferred_issue_exception);
    ...
    always_comb begin
        ...
        if (dot_deferred_rs1_match) begin
            ex_operand_a = deferred_load_data;
        end
        ...
        if (dot_deferred_rs2_match) begin
            ex_operand_b_reg = deferred_load_data;
        end
    ...
    // T2: branches are never admitted while deferred-load state is active;
    // keep their forwarding cone independent of the DOT-only deferred mux.
    always_comb begin
        branch_operand_a = id_ex_reg.operand_a;
        branch_operand_b = id_ex_reg.operand_b;

        if (ex_mem_reg.valid && ex_mem_reg.reg_write &&
            !ex_mem_reg.mem_to_reg && (ex_mem_reg.rd != 5'd0) &&
            (ex_mem_reg.rd == id_ex_reg.rs1)) begin
            branch_operand_a = ex_mem_reg.alu_result;
        end else if (wb_writes_rd && (mem_wb_reg.rd == id_ex_reg.rs1)) begin
            branch_operand_a = wb_write_data;
        end
        if (dot_rs1_completion_dependency) begin
            branch_operand_a = dot_complete_result;
        end

        if (ex_mem_reg.valid && ex_mem_reg.reg_write &&
            !ex_mem_reg.mem_to_reg && (ex_mem_reg.rd != 5'd0) &&
            (ex_mem_reg.rd == id_ex_reg.rs2)) begin
            branch_operand_b = ex_mem_reg.alu_result;
        end else if (wb_writes_rd && (mem_wb_reg.rd == id_ex_reg.rs2)) begin
            branch_operand_b = wb_write_data;
        end
        if (dot_rs2_completion_dependency) begin
            branch_operand_b = dot_complete_result;
        end
    end

    assign ex_branch_taken = id_ex_reg.valid &&
                             id_ex_reg.branch &&
                             (branch_operand_a == branch_operand_b);
    ...
    bram_instr_mem_validation #(
        ...
        .clk(clk),
        .validation_load_mode(validation_load_mode),
        .validation_prog_we(validation_prog_we),
        .validation_prog_addr(validation_prog_addr),
        .validation_prog_data(validation_prog_data),
        .addr(instruction_addr),
        .instruction(bram_instruction)
    );

    bram_data_mem_validation #(
        .DEPTH(DMEM_DEPTH),
        .WIDTH(32)
    ) data_mem_inst (
        .clk(clk),
        .validation_load_mode(validation_load_mode),
        .validation_data_we(validation_data_we),
        .validation_data_addr(validation_data_addr),
        .validation_data_wdata(validation_data_wdata),
        .validation_data_re(validation_data_re),
        .validation_data_raddr(validation_data_raddr),
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            load_overlap_reserved        <= 1'b0;
            deferred_load_valid           <= 1'b0;
            deferred_load_pc              <= 32'h0;
            deferred_load_opcode          <= OP_NOP;
            deferred_load_rd              <= 5'd0;
            deferred_load_data            <= 32'h0;
            deferred_load_addr            <= 32'h0;
            second_load_reserved          <= 1'b0;
            second_load_idex_owned        <= 1'b0;
            dot_idex_consumed             <= 1'b0;
            dot_consumed_instruction      <= NOP_INSTRUCTION;
            dot_consumed_pc              <= 32'h0;
            ...
            if (mem_wb_reg.valid && mem_wb_reg.mem_write)
            ...
            retire_valid <= dot_complete_valid ||
                            (!dot_complete_valid && deferred_load_valid) ||
                            (!dot_complete_valid && !deferred_load_valid &&
                             mem_wb_reg.valid && !dot_load_completion_deferred);
            if (dot_complete_valid) begin
                ...
                retire_mem_addr   <= 32'h0000_0000;
                retire_mem_data   <= 32'h0000_0000;
            end else if (deferred_load_valid) begin
                retire_pc         <= deferred_load_pc;
                retire_opcode     <= deferred_load_opcode;
                retire_rd         <= deferred_load_rd;
                retire_reg_write  <= (deferred_load_rd != 5'd0);
                retire_write_data <= deferred_load_data;
                retire_mem_write  <= 1'b0;
                retire_mem_addr   <= deferred_load_addr;
                retire_mem_data   <= deferred_load_data;
            end else begin
            ...
            if (dot_complete_valid || deferred_load_valid ||
                (!deferred_load_valid && mem_wb_reg.valid &&
                 !dot_load_completion_deferred)) begin
                retired_instructions <= retired_instructions + 32'd1;
            end

            if (dot_load_completion_deferred && !deferred_load_valid) begin
                deferred_load_valid  <= 1'b1;
                deferred_load_pc     <= mem_wb_reg.pc;
                deferred_load_opcode <= mem_wb_reg.opcode;
                deferred_load_rd     <= mem_wb_reg.rd;
                deferred_load_data   <= wb_write_data;
                deferred_load_addr   <= mem_wb_reg.alu_result;
            end else if (deferred_load_valid && !dot_complete_valid) begin
                deferred_load_valid <= 1'b0;
                load_overlap_reserved <= 1'b0;
            end
            ...
            if (dot_issue_accept) begin
                ...
                dot_chain_rd_reg   <= id_ex_reg.rd;
                dot_idex_consumed  <= 1'b1;
                dot_consumed_instruction <= id_ex_reg.instruction;
                dot_consumed_pc <= id_ex_reg.pc;
            end else if (!id_ex_is_dot ||
                         (id_ex_reg.instruction != dot_consumed_instruction) ||
                         (id_ex_reg.pc != dot_consumed_pc)) begin
                dot_idex_consumed  <= 1'b0;
            end
            if (load_overlap_transfer_fire) begin
                load_overlap_reserved <= 1'b1;
            end
            if (second_load_transfer_fire) begin
                second_load_reserved <= 1'b1;
                second_load_idex_owned <= 1'b1;
            end
            if (second_load_idex_owned && id_ex_reg.valid &&
                (id_ex_reg.opcode == OP_LOAD) &&
                !second_load_holding && !scalar_overlap_hold) begin
                second_load_idex_owned <= 1'b0;
            end
            if (second_load_reserved && mem_wb_reg.valid &&
                (mem_wb_reg.opcode == OP_LOAD) &&
                !dot_load_completion_deferred &&
                !deferred_load_valid) begin
                second_load_reserved <= 1'b0;
            end
            ...
            end // !scalar_overlap_hold
            // H1.3b atomic exception: consume exactly one qualifying LOAD B
            // from IF/ID into ID/EX while the scalar stages remain held.
            // No EX/MEM or MEM/WB state advances on this edge.
            else if (second_load_transfer_fire) begin
                id_ex_reg.valid       <= 1'b1;
                id_ex_reg.pc          <= if_id_reg.pc;
                id_ex_reg.instruction <= if_id_reg.instruction;
                id_ex_reg.opcode      <= if_id_opcode;
                ...
                id_ex_reg.accumulator <= decode_accumulator;
                id_ex_reg.reg_write   <= stagec_opcode_writes_rd(if_id_opcode);
                id_ex_reg.mem_read    <= 1'b1;
                id_ex_reg.mem_write   <= 1'b0;
                id_ex_reg.mem_to_reg  <= 1'b1;
                id_ex_reg.use_imm     <= 1'b1;
                id_ex_reg.branch      <= 1'b0;
                id_ex_reg.jump        <= 1'b0;
                if_id_reg.valid       <= 1'b0;
            end
            if (dot_load_completion_deferred) begin
                // Consume the normal LOAD payload on the capture edge.  The
                // scalar stages remain held until the deferred entry drains.
                clear_mem_wb();
            end
        end else begin
            retire_valid     <= 1'b0;
    ...
    always @(posedge clk) begin
        ...
        if (!rst) begin
            assert (!(dot_assert_normal_wb_request && dot_assert_dot_wb_request) ||
                    dot_load_completion_deferred || deferred_load_valid)
                else $fatal(1, "normal/DOT collision was not deferred");
            assert (!(dot_assert_normal_retire_request && dot_assert_consumed_valid) ||
                    dot_load_completion_deferred || deferred_load_valid)
                else $fatal(1, "normal/DOT retirement was not deferred");
            assert (!rf_write_enable ||
                    ((normal_wb_valid || dot_wb_valid || deferred_load_valid) &&
                     (rf_write_addr != 5'd0)))
            ...
            assert (!(dot_idex_consumed && dot_issue_accept &&
                      (dot_consumed_pc == id_ex_reg.pc) &&
                      (dot_consumed_instruction == id_ex_reg.instruction)))
                else $fatal(1, "same dynamic DOT instruction accepted twice");
            assert (!dot_load_completion_deferred || dot_wb_valid ||
                    !rf_write_enable)
                else $fatal(1, "deferred LOAD was not suppressed on capture");
            assert (!deferred_load_valid || !normal_wb_valid)
                else $fatal(1, "normal completion remained valid with deferred LOAD");
            assert (!deferred_load_valid || load_overlap_reserved)
                else $fatal(1, "deferred LOAD lost its overlap ownership");
            if (dot_deferred_rs1_match) begin
                assert (ex_operand_a == deferred_load_data)
                    else $fatal(1, "deferred rs1 forwarding mismatch");
                assert (!(ex_mem_reg.valid && ex_mem_reg.reg_write &&
                          !ex_mem_reg.mem_to_reg &&
                          ex_mem_reg.rd == id_ex_reg.rs1))
                    else $fatal(1, "deferred rs1 coexists with EX/MEM producer");
                assert (!(wb_writes_rd && mem_wb_reg.rd == id_ex_reg.rs1))
                    else $fatal(1, "deferred rs1 coexists with MEM/WB producer");
                assert (!dot_rs1_completion_dependency)
                    else $fatal(1, "deferred rs1 coexists with DOT completion");
            end
            if (dot_deferred_rs2_match) begin
                assert (ex_operand_b_reg == deferred_load_data)
                    else $fatal(1, "deferred rs2 forwarding mismatch");
                assert (!(ex_mem_reg.valid && ex_mem_reg.reg_write &&
                          !ex_mem_reg.mem_to_reg &&
                          ex_mem_reg.rd == id_ex_reg.rs2))
                    else $fatal(1, "deferred rs2 coexists with EX/MEM producer");
                assert (!(wb_writes_rd && mem_wb_reg.rd == id_ex_reg.rs2))
                    else $fatal(1, "deferred rs2 coexists with MEM/WB producer");
                assert (!dot_rs2_completion_dependency)
                    else $fatal(1, "deferred rs2 coexists with DOT completion");
            end
            assert (!dot_deferred_issue_exception || id_ex_is_dot)
                else $fatal(1, "deferred exception released non-DOT state");
            assert (!(deferred_load_valid && id_ex_reg.valid && id_ex_reg.branch))
                else $fatal(1, "branch reached EX while deferred LOAD was pending");

            // H1.3a: a second dynamic LOAD may be admitted once, then is
            // held in ID/EX while the single deferred completion entry is
            // occupied.  It must not reach EX/MEM or initiate a BRAM read.
            assert (!(second_load_reserved && second_load_admit_allowed))
                else $fatal(1, "second LOAD admitted more than once");
            assert (!second_load_transfer_fire || second_load_admit_allowed)
                else $fatal(1, "second LOAD transfer fired without admission");
            assert (!second_load_transfer_fire ||
                    (if_id_reg.valid && if_id_opcode_valid &&
                     (if_id_opcode == OP_LOAD)))
                else $fatal(1, "non-LOAD consumed by second LOAD transfer");
            assert (!second_load_holding || scalar_overlap_hold)
                else $fatal(1, "held second LOAD escaped overlap hold");
            assert (!second_load_holding || !data_mem_read_en)
                else $fatal(1, "held second LOAD initiated a BRAM read");
            assert (!second_load_holding ||
                    !(ex_mem_reg.valid && (ex_mem_reg.pc == id_ex_reg.pc)))
                else $fatal(1, "held second LOAD transferred to EX/MEM");
            assert (!(second_load_holding && dot_load_completion_deferred))
                else $fatal(1, "held second LOAD created a completion");
            assert (!(dot_load_completion_deferred && deferred_load_valid))
                else $fatal(1, "deferred completion entry overwritten");
            assert (!(second_load_reserved && if_id_opcode_valid &&
                      (if_id_opcode == OP_LOAD) && second_load_admit_allowed))
                else $fatal(1, "third LOAD entered overlap mechanism");

            // After architectural completion the final DOT metadata may
            // drain while the wrapper disables execution; that is expected
            // and must not be mistaken for a disabled-pipeline mutation.
            if (!rst && !enable && dot_assert_shadow_valid && !validation_done) begin
            ...
            end else if ((!rst || !dot_assert_shadow_valid) && !validation_done) begin
