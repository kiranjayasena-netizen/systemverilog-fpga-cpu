...
module cpu_core_pipeline_targetbuf #(
    ...
    // Phase 13F keeps the Phase 13C WB-to-ID bypass behaviour, but splits
    ...
    // Timing-safe Phase 13F scope: consume the one-entry target buffer only
    // for the ID-stage fast JUMP path. Allowing EX-stage BEQ redirects to
    // hit this buffer placed branch/load forwarding logic onto the
    // instruction-BRAM address path and failed timing. Taken BEQ targets may
    // still fill the buffer for later reuse, but BEQ redirection keeps the
    // registered Phase 13E path.
    ...
    always_comb begin
        if (redirect_target_buffer_hit) begin
            // A buffered control-flow target can be consumed immediately. At
            // the same clock edge, request the sequential target+4 word.
            next_instruction_addr = redirect_target + 32'd4;
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            if (ex_redirect_taken) begin
                if (redirect_target_buffer_hit) begin
                    if_id_reg.valid       <= opcode_is_valid(target_buffer_instruction[31:28]);
                    if_id_reg.pc          <= redirect_target;
                    if_id_reg.instruction <= target_buffer_instruction;

                    fetch_buffer_valid       <= 1'b0;
                    fetch_buffer_is_target   <= 1'b0;
                    fetch_pending_pc         <= redirect_target + 32'd4;
                    fetch_pending_valid      <= 1'b1;
                    fetch_pending_is_target  <= 1'b0;
                    redirect_pending_valid   <= 1'b0;
                    jump_response_pending    <= 1'b0;
                    fetch_pc                 <= redirect_target + 32'd8;
                end else begin
                    if_id_reg.valid       <= 1'b0;
                ...
                if (redirect_target_buffer_hit) begin
                    ...
                    fetch_buffer_valid       <= 1'b0;
                    fetch_buffer_is_target   <= 1'b0;
                    fetch_pending_pc         <= id_jump_target + 32'd4;
                    fetch_pending_valid      <= 1'b1;
                    fetch_pending_is_target  <= 1'b0;
                    jump_response_pending    <= 1'b0;
                    fetch_pc                 <= id_jump_target + 32'd8;
                end else begin
                    if_id_reg.valid       <= 1'b0;
