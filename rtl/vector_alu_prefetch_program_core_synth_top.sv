// Stage 10B physical-characterisation wrapper.  Instantiates the complete
// ALU/read-overlap core and exposes compact status while retaining datapath.
module vector_alu_prefetch_program_core_synth_top(
 ...
 vector_alu_prefetch_program_core core(
  ...
  .debug_read_enable(debug_read_enable),.debug_read_addr(debug_read_addr),.debug_read_data(debug_read_data),.state_debug(state_debug),.prefetch_request_count(prefetch_request_count),.prefetch_hit_count(prefetch_hit_count),.prefetch_miss_count(prefetch_miss_count),.entry0_hit_count(entry0_hit_count),.entry1_hit_count(entry1_hit_count),.overlap_alu_count(overlap_alu_count),.overlap_store_count(overlap_store_count),.dependency_blocked_count(dependency_blocked_count),.entry0_fill_count(entry0_fill_count),.entry1_fill_count(entry1_fill_count),.duplicate_suppression_count(duplicate_suppression_count),.alu_read_overlap_count(alu_read_overlap_count),.alu_prefetch_hit_count(alu_prefetch_hit_count),.debug_entry0_valid(debug_entry0_valid),.debug_entry1_valid(debug_entry1_valid),.debug_entry0_addr(debug_entry0_addr),.debug_entry1_addr(debug_entry1_addr),.debug_entry0_data(debug_entry0_data),.debug_entry1_data(debug_entry1_data),.debug_pending_read_valid(debug_pending_read_valid));
 assign status_parity=^current_instruction ^ current_instruction_valid ^ ^data_debug_read_data ^ ^debug_read_data ^ ^prefetch_request_count ^ ^prefetch_hit_count ^ ^alu_read_overlap_count ^ ^alu_prefetch_hit_count ^ ^debug_entry0_data ^ ^debug_entry1_data;
endmodule
