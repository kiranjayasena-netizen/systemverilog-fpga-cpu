module basys3_vector_gpu_top (
  ...
  logic pwe,dwe; logic [3:0] pa; logic [15:0] pd; logic [4:0] da,plen; logic [127:0] dd;
  ...
  (* DONT_TOUCH = "yes" *) vector_gpu_accelerator gpu(.clk(gpu_clk),.rst(rst_btn|gpu_rst),.start(gpu_start),.program_length(plen),.prog_write_enable(pwe),.prog_write_addr(pa),.prog_write_data(pd),.data_write_enable(dwe),.data_write_addr(da),.data_write_data(dd),.data_read_enable(1'b0),.data_read_addr(5'b0),.data_read_data(),.reg_read_enable(1'b0),.reg_read_addr(3'b0),.reg_read_data(),.busy(gpu_busy),.done(gpu_done),.cycle_count(cycles),.prefetch_request_count(req),.prefetch_hit_count(hit),.alu_read_overlap_count(ar),.alu_prefetch_hit_count(aph));
