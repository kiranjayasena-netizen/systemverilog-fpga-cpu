...
module cpu_core_pipeline_forwardtiming_phase21 #(
    ...
    assign ex_branch_mispredict_not_taken = id_ex_reg.valid &&
                                            id_ex_reg.branch &&
                                            id_ex_reg.predicted_branch_taken &&
                                            !ex_branch_taken;
    assign ex_redirect_taken = (ex_branch_taken && !id_ex_reg.predicted_branch_taken) ||
                               ex_jump_taken ||
                               ex_branch_mispredict_not_taken;
    assign ex_redirect_target = ex_branch_mispredict_not_taken
                              ? (id_ex_reg.pc + 32'd4)
                              : (id_ex_reg.pc + (id_ex_reg.imm_ext << 2));
    assign id_jump_target = if_id_reg.pc + (if_id_imm_ext << 2);
    assign id_beq_predict_target = if_id_reg.pc + (if_id_imm_ext << 2);
    ...
    assign id_beq_predict_request = if_id_reg.valid &&
                                    if_id_opcode_valid &&
                                    (if_id_opcode == OP_BEQ) &&
                                    if_id_imm13[12] &&
                                    !decode_stall &&
    ...
    assign id_beq_predict_redirect = id_beq_predict_request && !ex_redirect_taken && !id_jump_redirect;
    assign redirect_taken  = ex_redirect_taken || id_jump_redirect || id_beq_predict_redirect;
    assign redirect_target = ex_redirect_taken ? ex_redirect_target :
                             id_jump_redirect ? id_jump_target :
                             id_beq_predict_target;
    ...
    always_comb begin
        ...
        end else if (id_beq_predict_request) begin
            next_instruction_addr = id_beq_predict_target;
    ...
    task automatic clear_id_ex();
        begin
            ...
            id_ex_reg.jump        <= 1'b0;
            id_ex_reg.predicted_branch_taken <= 1'b0;
        end
    endtask
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            if (ex_redirect_taken || decode_stall) begin
                ...
                id_ex_reg.predicted_branch_taken <= id_beq_predict_redirect;
            end else begin
                bubble_id_ex();
            end
            ...
            end else if (id_beq_predict_redirect) begin
                if_id_reg.valid       <= 1'b0;

                fetch_buffer_valid       <= 1'b0;
                fetch_pending_valid      <= 1'b0;
                jump_response_pending    <= 1'b1;
                jump_response_pc         <= id_beq_predict_target;
                fetch_pc                 <= id_beq_predict_target + 32'd4;
            end else if (redirect_pending_valid) begin
                if_id_reg.valid       <= 1'b0;
