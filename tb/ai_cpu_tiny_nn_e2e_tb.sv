...
module ai_cpu_tiny_nn_e2e_tb;
  ...
  ai_cpu_validation_wrapper #(.OPTIMIZED(0),.STAGE31_COMPLETION(1)) b(.clk,.rst,.load_mode(lm_b),.start(start_b),.prog_we(pw_b),.prog_addr(pa_b),.prog_data(pd_b),.data_we(dw_b),.data_addr_host(da_b),.data_wdata(dd_b),.data_re(dr_b),.data_raddr(ra_b),.data_rdata(rd_b),.busy(busy_b),.done(done_b),.cycle_count(cyc_b));
  ai_cpu_validation_wrapper #(.OPTIMIZED(1),.STAGE31_COMPLETION(1)) o(.clk,.rst,.load_mode(lm_o),.start(start_o),.prog_we(pw_o),.prog_addr(pa_o),.prog_data(pd_o),.data_we(dw_o),.data_addr_host(da_o),.data_wdata(dd_o),.data_re(dr_o),.data_raddr(ra_o),.data_rdata(rd_o),.busy(busy_o),.done(done_o),.cycle_count(cyc_o));
  ...
  task automatic run_variant(input bit opt,input integer n,input integer t,output [31:0] y0,output [31:0] y1,output [31:0] cls,output [31:0] cycles);
    ...
    begin
      ...
      if(opt) begin lm_o<=1; for(k=0;k<n;k=k+1) put_prog(1,k,op[k]); put_data(1,16,inpack); put_data(1,20,32'h01010101); put_data(1,21,32'hff01ff01); put_data(1,22,32'h01ff01ff); put_data(1,23,32'hffff0101); put_data(1,30,32'h80000000); lm_o<=0;@(posedge clk);start_o<=1;@(posedge clk);start_o<=0; timeout=0; while(done_o!==1 && timeout<10000) begin @(posedge clk); timeout=timeout+1; end if(done_o!==1) $fatal(1,"optimized timeout"); cycles=cyc_o; repeat(3)@(posedge clk);lm_o<=1;get_data(1,40,y0);get_data(1,41,y1);get_data(1,42,cls);end
      else begin lm_b<=1; for(k=0;k<n;k=k+1) put_prog(0,k,bp[k]); for(k=0;k<4;k=k+1) put_data(0,16+k,x[k]); put_data(0,30,32'h80000000); lm_b<=0;@(posedge clk);start_b<=1;@(posedge clk);start_b<=0; timeout=0; while(done_b!==1 && timeout<10000) begin @(posedge clk); timeout=timeout+1; end if(done_b!==1) $fatal(1,"baseline timeout"); cycles=cyc_b; repeat(3)@(posedge clk);lm_b<=1; $display("MEM40=%h MEM41=%h MEM42=%h",b.g_baseline.core.data_mem_inst.mem[40],b.g_baseline.core.data_mem_inst.mem[41],b.g_baseline.core.data_mem_inst.mem[42]); get_data(0,40,y0);get_data(0,41,y1);get_data(0,42,cls);end
    end
  endtask
  initial begin
    ...
    for(j=0;j<4;j=j+1) begin
      ...
      run_variant(0,141,j,b0,b1,bc,cb); run_variant(1,86,j,o0,o1,oc,co);
    ...
    $display("AI_TINY_NN_LAYER1_PASS");
    $display("AI_TINY_NN_COMPLETION_PROTOCOL_PASS");
