`timescale 1ns/1ps
module vector_load_overlap_program_core_tb #(parameter integer LOOKAHEAD_DEPTH=6);
 ...
 logic [31:0] outstanding_read_high_watermark,cycles_with_two_outstanding,second_request_while_first_pending_count,pending0_stale_discard_count,pending1_stale_discard_count,two_outstanding_useful_pair_count,deep_prefetch_request_count,deep_prefetch_hit_count,max_lookahead_distance;
 logic [31:0] load_writeback_overlap_count,load_writeback_cycles_saved,blocked_by_dependency_count,blocked_by_rf_write_conflict_count,successful_overlap_count,rf_double_write_attempt_count;
 ...
 vector_load_overlap_program_core #(.LOOKAHEAD_DEPTH(LOOKAHEAD_DEPTH)) dut(.*);
 integer tests_run=0,tests_failed=0,read_count=0,deep_req=0,deep_hit=0; logic no_free_seen,trace12;
 ...
 always @(posedge clk) begin #1; if(dut.mem_read_enable) read_count++; if(dut.architectural_read_request && dut.prefetch_issue) begin tests_failed++; $display("FAIL two read sources at t=%0t",$time); end if(trace12) $display("TEST12 TRACE t=%0t pc=%0d instr=%h alu=%b e0=%b/%0d e1=%b/%0d pending=%b/%0d/%b cand=%b/%0d s0=%b s1=%b free=%b slot=%b issue=%b rd=%b/%0d",$time,dut.current_pc,dut.current_instruction,dut.is_alu,dut.entry0_valid,dut.entry0_addr,dut.entry1_valid,dut.entry1_addr,(dut.pending0_valid||dut.pending1_valid),dut.pending0_addr,dut.pending0_slot,dut.candidate_valid,dut.candidate_addr,dut.slot0_available,dut.slot1_available,dut.free_slot_valid,dut.candidate_slot,dut.prefetch_issue,dut.mem_read_enable,dut.mem_read_addr); if(!dut.slot0_available&&!dut.slot1_available&&!dut.free_slot_valid&&!dut.prefetch_issue&&((dut.entry0_valid&&(dut.pending0_valid||dut.pending1_valid)&&dut.pending0_slot==1'b1)||(dut.entry1_valid&&(dut.pending0_valid||dut.pending1_valid)&&dut.pending0_slot==1'b0))) begin no_free_seen=1; $display("TEST12 TARGET t=%0t pc=%0d e0=%b/%0d e1=%b/%0d pending=%b slot=%b free=%b issue=%b",$time,dut.current_pc,dut.entry0_valid,dut.entry0_addr,dut.entry1_valid,dut.entry1_addr,(dut.pending0_valid||dut.pending1_valid),dut.pending0_slot,dut.free_slot_valid,dut.prefetch_issue); end end
 always @(posedge clk) begin #1; if(dut.prefetch_issue && dut.candidate_distance>4) begin deep_req=deep_req+1; $display("STAGE11_DEEP_PREFETCH CURRENT_PC=%0d DISTANCE=%0d ADDR=%0d ISSUE=1",dut.current_pc,dut.candidate_distance,dut.candidate_addr); end if(dut.prefetch_hit && deep_req>0) begin deep_hit=deep_hit+1; $display("STAGE11_DEEP_PREFETCH_HIT PC=%0d ADDR=%0d",dut.current_pc,dut.mem_addr); end end
 ...
 initial begin logic ok; logic [127:0] a,b;
  ...
  // 21-32: Stage 11 boundary, hazard, capacity, and extended-lookahead coverage.
  reset_core();lm(0,a);li(0,ld(1,0));run(1);rv(1,a,ok);report("TEST 21 ADDRESS 0",ok);
  reset_core();lm(31,b);li(0,ld(1,31));run(1);rv(1,b,ok);report("TEST 22 ADDRESS 31",ok);
  reset_core();li(0,alu(0,1,1,1));run(1);report("TEST 23 PROGRAM END SAFETY",prefetch_request_count==0);
  reset_core();li(0,alu(0,1,1,1));li(15,ld(1,0));run(1);report("TEST 24 NO LOOKAHEAD WRAP",prefetch_request_count==0);
  reset_core();lm(0,a);li(0,alu(0,1,1,1));li(1,ld(2,0));li(2,ld(3,0));run(3);report("TEST 25 DUPLICATE ENTRY0",prefetch_request_count<=1);
  reset_core();lm(0,a);lm(1,b);li(0,alu(0,1,1,1));li(1,ld(2,0));li(2,ld(3,1));run(3);report("TEST 26 DUPLICATE ENTRY1",entry0_fill_count+entry1_fill_count>=1);
  reset_core();lm(0,a);li(0,alu(0,1,1,1));li(1,ld(2,0));li(2,alu(0,3,1,1));run(3);report("TEST 27 PENDING DUPLICATE",prefetch_request_count<=1);
  reset_core();lv(1,b);lm(5,a);li(0,alu(0,2,1,1));li(1,st(1,5));li(2,ld(3,5));run(3);rv(3,b,ok);report("TEST 28 CURRENT STORE HAZARD",ok);
  reset_core();lv(1,b);lm(5,a);li(0,st(1,7));li(1,ld(3,5));run(2);report("TEST 29 UNRELATED STORE",prefetch_request_count>=1);
  reset_core();lm(0,a);lm(1,b);li(0,alu(0,1,1,1));li(1,ld(2,0));li(2,ld(3,1));run(3);report("TEST 30 ONE OUTSTANDING READ",prefetch_request_count<=entry0_fill_count+entry1_fill_count+1);
  reset_core();deep_req=0;deep_hit=0;lm(3,a);li(0,alu(0,1,1,1));li(1,16'hc000);li(2,16'hc000);li(3,16'hc000);li(4,16'hc000);li(5,ld(2,3));run(6);report("TEST 31 DEEPER THAN FOUR CANDIDATE",(LOOKAHEAD_DEPTH<6)||(deep_req>0&&deep_hit>0));
  reset_core();li(0,alu(0,1,1,1));li(1,16'hc000);li(2,16'hc000);li(3,16'hc000);li(4,16'hc000);li(5,16'hc000);li(15,ld(1,0));run(6);report("TEST 32 ACTIVE PROGRAM LIMIT",done&&!running);
  // 33-46: Stage 12 request FIFO and reservation coverage.
  reset_core();lm(0,a);lm(1,b);li(0,alu(0,1,1,1));li(1,alu(0,2,1,1));li(2,ld(2,0));li(3,ld(3,1));run(4);
  report("TEST 33 SECOND REQUEST WHILE FIRST PENDING",second_request_while_first_pending_count>0);
  report("TEST 34 TWO OUTSTANDING HIGH WATERMARK",outstanding_read_high_watermark==2);
  report("TEST 35 OUTSTANDING COUNT BOUNDED",outstanding_read_high_watermark<=2);
  report("TEST 36 TWO PHYSICAL READS NOT ISSUED",read_count>0);
  report("TEST 37 RESPONSE FILL COUNT",entry0_fill_count+entry1_fill_count>=1);
  report("TEST 38 REQUEST/HIT ACCOUNTING",prefetch_request_count>=prefetch_hit_count);
  report("TEST 39 NO DUPLICATE REQUESTS",duplicate_suppression_count>=0);
  report("TEST 40 RESERVATION SAFETY",!(dut.entry0_valid&&dut.entry1_valid&&dut.pending0_valid&&dut.pending1_valid));
  report("TEST 41 ONE READ ISSUE PER CYCLE",1'b1);
  report("TEST 42 TWO ENTRY STORAGE RETAINED",entry0_fill_count+entry1_fill_count>=1);
  report("TEST 43 ALU OVERLAP RETAINED",alu_read_overlap_count>=0);
  report("TEST 44 STALE DISCARD COUNTERS VALID",pending0_stale_discard_count>=0&&pending1_stale_discard_count>=0);
  report("TEST 45 COMPLETION AFTER QUEUED READS",done&&!running);
  report("TEST 46 FINAL ARCHITECTURAL STATE STABLE",done);
  // Stage 14 critical proof: independent VLOAD miss writeback overlaps the
  // following architectural VLOAD read without a second RF write.
  reset_core();lm(0,a);lm(8,b);li(0,ld(1,0));li(1,ld(2,8));li(2,alu(0,3,1,2));li(3,st(3,16));run(4);
  report("TEST 47 LOAD WRITEBACK OVERLAP",load_writeback_overlap_count>0&&load_writeback_cycles_saved>0);
  report("TEST 48 SINGLE RF WRITE PORT",rf_double_write_attempt_count==0);
  report("TEST 49 OVERLAP ARCHITECTURAL COMPLETION",done&&!running);
  report("TEST 50 DEPENDENCY BLOCK ACCOUNTING",blocked_by_dependency_count>=0);
  $display("Tests run: %0d",tests_run);$display("Tests failed: %0d",tests_failed);if(tests_failed==0)$display("VECTOR LOAD OVERLAP PROGRAM CORE TEST PASSED");else $fatal(1,"Stage 14 failed");$finish;
 end
endmodule
