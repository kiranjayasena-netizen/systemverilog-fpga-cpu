...
module tb_cpu_core_multicycle_bram_full_programs;
    ...
    task automatic check_instruction_counts(
        ...
        begin
            check_count({active_subtest, ": active cycle count"}, cycles, expected_cycles);
    ...
    initial begin
        $dumpfile("tb_cpu_core_multicycle_bram_full_programs.vcd");
        $dumpvars(0, tb_cpu_core_multicycle_bram_full_programs);

        $display("Starting Phase 10G BRAM-aware multi-cycle CPU full custom-ISA verification...");
        ...
        start_subtest("BRAM arithmetic edge program");
        ...
        check_instruction_counts(48, 9, 0, 0, 0, 0, 1, 0);
        finish_subtest();

        start_subtest("BRAM memory offset program");
        ...
        check_common_control_safety();
        check_instruction_counts(49, 3, 3, 2, 0, 0, 1, 0);
        finish_subtest();

        start_subtest("BRAM branch control program");
        ...
        check_common_control_safety();
        check_instruction_counts(46, 7, 0, 0, 2, 0, 1, 0);
        finish_subtest();

        start_subtest("BRAM jump control program");
        ...
        check_common_control_safety();
        check_instruction_counts(31, 4, 0, 0, 0, 2, 1, 0);
        finish_subtest();

        start_subtest("BRAM simple loop program");
        ...
        check_common_control_safety();
        check_instruction_counts(73, 9, 0, 1, 3, 2, 1, 0);
        finish_subtest();

        start_subtest("BRAM invalid opcode safety program");
        ...
        check_instruction_counts(24, 2, 0, 1, 0, 0, 1, 2);
        finish_subtest();

        $display("");
        $display("Aggregate Phase 10G performance:");
        ...
        check_count("aggregate: total active cycles", total_cycles, 271);
        ...
        if (tests_failed != 0) begin
            $display("PHASE 10G BRAM-AWARE FULL-PROGRAM TEST FAILED");
            $fatal(1, "%0d of %0d Phase 10G tests failed.", tests_failed, tests_run);
        end

        $display("PHASE 10G BRAM-AWARE FULL-PROGRAM TEST PASSED");
        $finish;
    end
