`timescale 1ns/1ps
module vector_two_outstanding_prefetch_program_core_tb #(parameter integer LOOKAHEAD_DEPTH=6);
 ...
 vector_two_outstanding_prefetch_program_core #(.LOOKAHEAD_DEPTH(LOOKAHEAD_DEPTH)) dut(.*);
 ...
 initial begin logic ok; logic [127:0] a,b;
  ...
  $display("Tests run: %0d",tests_run);$display("Tests failed: %0d",tests_failed);if(tests_failed==0)$display("VECTOR TWO OUTSTANDING PREFETCH PROGRAM CORE TEST PASSED");else $fatal(1,"Stage 12 failed");$finish;
 end
endmodule
