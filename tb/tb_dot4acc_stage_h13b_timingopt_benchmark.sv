...
module tb_dot4acc_stage_h13b_timingopt_benchmark;
    ...
    typedef struct {
        ...
        int h13_second_load_exmem_transfers;
        int h13_second_load_bram_reads;
        int h13_second_load_completions;
        int h13_dot_issue_both_ready;
        int first_issue;
        int last_issue;
    ...
    cpu_core_pipeline_dot4acc_memopt_h13b_timingopt #(
        .IMEM_DEPTH(IMEM_DEPTH),
        .DMEM_DEPTH(DMEM_DEPTH),
        .IMEM_INIT_FILE("")
    ) dot_dut (
    ...
    task automatic init_measurement(output measurement_t m);
        ...
        m.h13_second_load_exmem_transfers = 0;
        m.h13_second_load_bram_reads = 0;
        m.h13_second_load_completions = 0;
        m.h13_dot_issue_both_ready = 0;
        m.first_issue = -1;
        m.last_issue = -1;
    ...
    task automatic run_dot_to_store(output measurement_t m, input int max_cycles);
        ...
        while (!done && (timeout < max_cycles)) begin
            ...
            if (dot_dut.second_load_transfer_fire)
                m.h13_second_load_admitted++;
            if (dot_dut.second_load_idex_owned &&
                !dot_dut.second_load_holding &&
                dot_dut.id_ex_reg.valid && dot_dut.id_ex_reg.mem_read &&
                !dot_dut.scalar_overlap_hold)
                m.h13_second_load_exmem_transfers++;
            if (dot_dut.second_load_reserved && dot_dut.data_mem_read_en &&
                dot_dut.ex_mem_reg.valid && dot_dut.ex_mem_reg.mem_read)
                m.h13_second_load_bram_reads++;
            if (dot_dut.second_load_reserved && dot_dut.mem_wb_reg.valid &&
                dot_dut.mem_wb_reg.mem_to_reg &&
                !dot_dut.dot_load_completion_deferred)
                m.h13_second_load_completions++;
            if (dot_dut.dot_issue_accept && dot_dut.dot_operands_ready &&
                !dot_dut.dot_accumulator_dependency_stall)
                m.h13_dot_issue_both_ready++;
            if (dot_dut.id_ex_reg.valid &&
                (dot_dut.id_ex_reg.opcode == OP_LOAD) &&
                dot_dut.second_load_idex_owned && dot_dut.dot_frontend_hold)
                m.h13_second_load_idex_held++;
            if (dot_dut.id_ex_reg.valid &&
                (dot_dut.id_ex_reg.opcode == OP_LOAD) &&
                dot_dut.second_load_idex_owned && dot_dut.deferred_load_valid)
    ...
    task automatic benchmark_memory_fed();
        ...
        for (int index = 0; index < 4; index++) begin
            ...
            $display("H1.3 ownership N=%0d second_admitted=%0d second_idex_held=%0d precompletion_held=%0d",
                     ...
                     m.h13_second_load_idex_held,
                     m.h13_second_load_precompletion_held);
            $display("H1.3 transfer N=%0d exmem=%0d bram=%0d completion=%0d dot_both_ready=%0d",
                     n, m.h13_second_load_exmem_transfers,
                     m.h13_second_load_bram_reads,
                     m.h13_second_load_completions,
                     m.h13_dot_issue_both_ready);
