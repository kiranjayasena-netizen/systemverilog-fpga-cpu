...
module tb_cpu_core_pipeline_targetbuf;
    ...
    task automatic run_single_forward_jump_focus();
        ...
        $display("");
        $display("---- Phase 13F focused single forward JUMP ----");
        prepare_program();
    ...
    task automatic run_consecutive_jumps_focus();
        ...
        $display("");
        $display("---- Phase 13F focused consecutive JUMPs ----");
        prepare_program();
    ...
    task automatic run_backward_jump_loop_focus();
        ...
        $display("");
        $display("---- Phase 13F focused backward JUMP loop ----");
        prepare_program();
    ...
    task automatic run_pause_after_jump_request_focus();
        ...
        $display("");
        $display("---- Phase 13F focused pause after fast JUMP request ----");
        prepare_program();
    ...
    task automatic run_older_beq_younger_jump_focus();
        $display("");
        $display("---- Phase 13F focused older BEQ overrides younger JUMP ----");
        prepare_program();
    ...
    task automatic run_fetch_buffer_near_jump_focus();
        $display("");
        $display("---- Phase 13F focused load-use buffer near JUMP ----");
        prepare_program();
    ...
    initial begin
        $dumpfile("tb_cpu_core_pipeline_targetbuf.vcd");
        $dumpvars(0, tb_cpu_core_pipeline_targetbuf);
        ...
        if (tests_failed == 0) begin
            $display("Phase 13F targetbuf PIPELINE TEST PASSED");
        end else begin
            $display("Phase 13F targetbuf PIPELINE TEST FAILED");
            $fatal(1, "Phase 13F targetbuf pipeline test failed");
        end
