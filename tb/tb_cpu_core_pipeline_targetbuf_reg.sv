...
module tb_cpu_core_pipeline_targetbuf_reg;
    ...
    logic [31:0] target_buffer_hits;
    logic [31:0] target_buffer_misses;
    ...
    cpu_core_pipeline_targetbuf_reg #(
        ...
        .target_buffer_hits(target_buffer_hits),
        .target_buffer_misses(target_buffer_misses)
    );
    ...
    task automatic reset_core();
        ...
        check_equal32("reset target-buffer hits", target_buffer_hits, 32'd0);
        check_equal32("reset target-buffer misses", target_buffer_misses, 32'd0);
    endtask
    ...
    task automatic run_until_retired(input string program_name, input int expected_retired, input int max_cycles);
        ...
        if (retired_instructions != 0) begin
            ...
            $display("  Target-buffer hits:     %0d", target_buffer_hits);
            $display("  Target-buffer misses:   %0d", target_buffer_misses);
        end
    endtask
    ...
    task automatic run_single_forward_jump_focus();
        ...
        $display("");
        $display("---- Phase 13G focused single forward JUMP ----");
        prepare_program();
        ...
        check_equal32("single jump target-buffer miss count", target_buffer_misses, 32'd1);
        check_equal32("single jump target-buffer hit count", target_buffer_hits, 32'd0);
    ...
    task automatic run_consecutive_jumps_focus();
        ...
        $display("");
        $display("---- Phase 13G focused consecutive JUMPs ----");
        prepare_program();
    ...
    task automatic run_backward_jump_loop_focus();
        ...
        $display("");
        $display("---- Phase 13G focused backward JUMP loop ----");
        prepare_program();
        ...
        check_true("backward loop uses target buffer on repeated target", target_buffer_hits > 32'd0);
    endtask
    ...
    task automatic run_pause_after_jump_request_focus();
        ...
        $display("");
        $display("---- Phase 13G focused pause after fast JUMP request ----");
        prepare_program();
    ...
    task automatic run_older_beq_younger_jump_focus();
        $display("");
        $display("---- Phase 13G focused older BEQ overrides younger JUMP ----");
        prepare_program();
    ...
    task automatic run_fetch_buffer_near_jump_focus();
        $display("");
        $display("---- Phase 13G focused load-use buffer near JUMP ----");
        prepare_program();
    ...
    initial begin
        $dumpfile("tb_cpu_core_pipeline_targetbuf_reg.vcd");
        $dumpvars(0, tb_cpu_core_pipeline_targetbuf_reg);
        ...
        $display("  Final target-buffer hits:   %0d", target_buffer_hits);
        $display("  Final target-buffer misses: %0d", target_buffer_misses);
        ...
        if (tests_failed == 0) begin
            $display("Phase 13G targetbuf_reg PIPELINE TEST PASSED");
        end else begin
            $display("Phase 13G targetbuf_reg PIPELINE TEST FAILED");
            $fatal(1, "Phase 13G targetbuf_reg pipeline test failed");
        end
