...
module tb_cpu_core_multicycle_bram_prefetch_ctrlopt;
    ...
    localparam int unsigned PHASE11B_BASELINE_CYCLES = 179;
    localparam int unsigned PHASE11B_BASELINE_INSTR  = 58;
    ...
    task automatic check_less_or_equal(
        input string       case_name,
        input int unsigned actual,
        input int unsigned limit
    );
        begin
            ...
            if (actual > limit) begin
                tests_failed++;
                $error(
                    "FAIL: %s | expected less than or equal to %0d got=%0d",
                    case_name,
                    limit,
                    actual
                );
    ...
    initial begin
        $dumpfile("tb_cpu_core_multicycle_bram_prefetch_ctrlopt.vcd");
        $dumpvars(0, tb_cpu_core_multicycle_bram_prefetch_ctrlopt);

        $display("Starting Phase 11D BRAM-aware control-flow-optimised prefetch CPU verification...");
        ...
        start_subtest("Ctrlopt arithmetic edge program");
        ...
        check_less_or_equal("arithmetic: no regression versus Phase 11B prefetch baseline", cycles, 30);
        finish_subtest();

        start_subtest("Ctrlopt memory offset program");
        ...
        check_less_or_equal("memory: no regression versus Phase 11B prefetch baseline", cycles, 33);
        finish_subtest();

        start_subtest("Ctrlopt branch control program");
        ...
        check_less_than("branch: cycle count below Phase 11B prefetch baseline", cycles, 30);
        finish_subtest();

        start_subtest("Ctrlopt jump control program");
        ...
        check_less_than("jump: cycle count below Phase 11B prefetch baseline", cycles, 23);
        finish_subtest();

        start_subtest("Ctrlopt simple loop program");
        ...
        check_less_than("loop: cycle count below Phase 11B prefetch baseline", cycles, 49);
        finish_subtest();

        start_subtest("Ctrlopt invalid opcode safety program");
        ...
        check_less_or_equal("invalid: no regression versus Phase 11B prefetch baseline", cycles, 14);
        finish_subtest();

        $display("");
        $display("Aggregate Phase 11D performance:");
        ...
        $display("Phase 11B prefetch baseline: cycles=%0d instructions=%0d CPI=3.086 MIPS @100MHz=32.402",
                 PHASE11B_BASELINE_CYCLES,
                 PHASE11B_BASELINE_INSTR);
        ...
        check_less_than("aggregate: total active cycles below Phase 11B prefetch baseline", total_cycles, PHASE11B_BASELINE_CYCLES);
        ...
        if (tests_failed != 0) begin
            $display("PHASE 11D BRAM PREFETCH CTRLOPT TEST FAILED");
            $fatal(1, "%0d of %0d Phase 11D tests failed.", tests_failed, tests_run);
        end

        $display("PHASE 11D BRAM PREFETCH CTRLOPT TEST PASSED");
        $finish;
    end
