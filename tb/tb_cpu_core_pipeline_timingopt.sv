...
module tb_cpu_core_pipeline_timingopt;
    ...
    task automatic run_single_forward_jump_focus();
        ...
        $display("");
        $display("---- Phase 13C focused single forward JUMP ----");
        prepare_program();
    ...
    task automatic run_consecutive_jumps_focus();
        ...
        $display("");
        $display("---- Phase 13C focused consecutive JUMPs ----");
        prepare_program();
    ...
    task automatic run_backward_jump_loop_focus();
        ...
        $display("");
        $display("---- Phase 13C focused backward JUMP loop ----");
        prepare_program();
    ...
    task automatic run_pause_after_jump_request_focus();
        ...
        $display("");
        $display("---- Phase 13C focused pause after fast JUMP request ----");
        prepare_program();
    ...
    task automatic run_older_beq_younger_jump_focus();
        $display("");
        $display("---- Phase 13C focused older BEQ overrides younger JUMP ----");
        prepare_program();
    ...
    task automatic run_fetch_buffer_near_jump_focus();
        $display("");
        $display("---- Phase 13C focused load-use buffer near JUMP ----");
        prepare_program();
    ...
    initial begin
        $dumpfile("tb_cpu_core_pipeline_timingopt.vcd");
        $dumpvars(0, tb_cpu_core_pipeline_timingopt);
        ...
        if (tests_failed == 0) begin
            $display("PHASE 13C TIMINGOPT PIPELINE TEST PASSED");
        end else begin
            $display("PHASE 13C TIMINGOPT PIPELINE TEST FAILED");
            $fatal(1, "Phase 13C timingopt pipeline test failed");
        end
