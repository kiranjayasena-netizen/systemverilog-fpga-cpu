...
[CmdletBinding()]
param(
    [double[]] $CoarseFrequenciesMHz = @(100, 102, 104, 106, 108, 110),
    [int] $RequiredRepetitions = 5,
    [double] $MaximumFrequencyMHz = 120,
    [string] $VivadoPath = "",
    [string] $ReportName = "dot4acc_stage_f",
    [switch] $Resume
)
...
$TclScript = Join-Path $RepoRoot "scripts\run_vivado_impl_dot4acc_stage_f.tcl"
...
Write-Host ""
Write-Host "DOT4ACC Stage F default-flow sweep completed. Repetition labels are not placement seeds."
