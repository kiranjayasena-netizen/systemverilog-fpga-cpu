# Phase 13E performance-directive implementation experiment.
#
# Run from the repository root in Vivado Tcl mode:
#   vivado -mode batch -source scripts/run_vivado_impl_pipeline_forwardtiming.tcl
#
# Optional PowerShell period override:
#   $env:PHASE13E_PERIOD="9.100"; vivado -mode batch -source scripts/run_vivado_impl_pipeline_forwardtiming.tcl
#
# This script keeps the Phase 13E RTL unchanged and evaluates a separate
# Vivado implementation strategy under reports/phase13e_timing/perf_directive_*.

set FPGA_PART "xc7a35tcpg236-1"
set XDC_FILE "constraints/basys3.xdc"
set TOP_MODULE "fpga_top_pipeline_forwardtiming"

if {[info exists ::env(PHASE13E_PERIOD)] && $::env(PHASE13E_PERIOD) ne ""} {
    set PERIOD $::env(PHASE13E_PERIOD)
} else {
    set PERIOD "9.100"
}

set PERIOD_TAG [string map {. p} $PERIOD]
set REPORT_ROOT "reports/phase13e_timing/perf_directive_${PERIOD_TAG}ns"
